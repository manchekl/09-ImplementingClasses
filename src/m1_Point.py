...
class Point (object):
    """Objects are points with x and y coordinates"""
    ...
    def move_to(self, dx, dy):
        distance=self.get_distance_from(Point(dx,dy))
        self.x=dx
        self.y=dy
    ...
    def move_by(self, dx, dy):
        distance = self.get_distance_from(Point(self.x+dx,self.y+dy))
    ...
    def get_distance_from(self,Point2):
        x=(abs(self.x-Point2.x))
        y=(abs(self.y-Point2.y))
        dist=((x**2)+(y**2))**0.5
        return dist

    def get_distance_from_start(self):
        s=Point(self.startx,self.starty)
        tot=self.get_distance_from(s)
        return tot
    ...
    def closer_to(self,Point2,Point3):
        calc1=self.get_distance_from(Point2)
        calc2=self.get_distance_from(Point3)
    ...
    def halfway_to(self,Point2):
        newx=(self.x+Point2.x)/2
        newy=(self.y+Point2.y)/2
